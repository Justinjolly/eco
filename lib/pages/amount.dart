...
Future<void> _fetchData() async {
  ...
  final snapshot = await FirebaseFirestore.instance.collection('amount').doc('Wms6j9aqramDy7gysC8G').get();
  ...
  setState(() {
    ...
    for (var i = 0; i < data!['splitAmounts'].length; i++) {
      ...
      if (member != null) {
        users.add({'name': member.toString()}); // Add 'member' as a map with 'name' key
      }
    }

    print(users);
    totalAmount = data['splitAmounts'][0]['amount']; // Get 'amount' from the first splitAmount mapping
  });
}
